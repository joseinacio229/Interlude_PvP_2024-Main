...
public class RaidBossSpawnManager
{
		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			...
			while (rset.next())
			{
				...
				if (template != null)
				{
					...
					spawnDat.setRespawnTimeSaved(rset.getLong("respawn_time"));
	...
	public void updateStatus(RaidBoss boss, boolean isBossDead)
	{
		...
		if (isBossDead)
		{
			...
			info.set("respawnTimeSaved", respawnTime);
	...
	public void reloadBossFromDB(int bossId)
	{
		try (Connection con = L2DatabaseFactory.getInstance().getConnection()) 
		{
			PreparedStatement statement = con.prepareStatement("SELECT * FROM raidboss_spawnlist WHERE boss_id = ?");
			statement.setInt(1, bossId);
			ResultSet rset = statement.executeQuery();
			
			if (rset.next()) 
			{
				final L2Spawn spawnDat = _spawns.get(bossId);
				if (spawnDat != null) 
				{
					spawnDat.setRespawnTimeSaved(rset.getLong("respawn_time"));
					// Atualize qualquer outro dado relevante aqui
				}
			}
		...
		{
			_log.warning("RaidBossSpawnManager: Couldn't reload raidboss #" + bossId + " from DB.");
		}
	}
