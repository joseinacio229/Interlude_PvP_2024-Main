...
public final class UseItem extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (Config.ENABLE_KILLS_LIMIT_D && activeChar.getPvpKills() < Config.PVP_D_GRADE && item.isEquipable())
		{
			if (item.getItem().getCrystalType() == CrystalType.D) {
				//activeChar.sendPacket(SystemMessageId.GRADE_PENALTY_FOR_PVP);
				activeChar.sendPacket(new PlaySound("ItemSound3.sys_fishing_failed"));
				activeChar.sendMessage("You need " + Config.PVP_D_GRADE + " pvp kills to equip D Grade item.");
				return;
			}
		}
		
		if (Config.ENABLE_KILLS_LIMIT_C && activeChar.getPvpKills() < Config.PVP_C_GRADE && item.isEquipable())
		{
			if (item.getItem().getCrystalType() == CrystalType.C) {
				//activeChar.sendPacket(SystemMessageId.GRADE_PENALTY_FOR_PVP);
				activeChar.sendPacket(new PlaySound("ItemSound3.sys_fishing_failed"));
				activeChar.sendMessage("You need " + Config.PVP_C_GRADE + " pvp kills to equip C Grade item.");
				return;
			}
		}
		
		if (Config.ENABLE_KILLS_LIMIT_B && activeChar.getPvpKills() < Config.PVP_B_GRADE && item.isEquipable())
		{
			if (item.getItem().getCrystalType() == CrystalType.B) {
				//activeChar.sendPacket(SystemMessageId.GRADE_PENALTY_FOR_PVP);
				activeChar.sendPacket(new PlaySound("ItemSound3.sys_fishing_failed"));
				activeChar.sendMessage("You need " + Config.PVP_B_GRADE + " pvp kills to equip B Grade item.");
				return;
			}
		}
		
		if (Config.ENABLE_KILLS_LIMIT_A && activeChar.getPvpKills() < Config.PVP_A_GRADE
			&& item.isEquipable())
		{
		...
		if (Config.ENABLE_KILLS_LIMIT_S && activeChar.getPvpKills() < Config.PVP_S_GRADE
			&& item.isEquipable())
		{
