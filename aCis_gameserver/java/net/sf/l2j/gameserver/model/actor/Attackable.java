...
public class Attackable extends Npc
{
	...
	protected final List<IntIntHolder> _sweepItems = new ArrayList<>();
			...
			if (attackerParty == null)
			{
				...
				if (!attacker.isDead() && attacker.getKnownType(Attackable.class).contains(this))
				{
					...
					if (attacker instanceof Player)
					{
					    ...
					    {
					        player.addExpAndSp(exp, sp, rewards);
					    } else {
					        player.addExpAndSp(0, 0, rewards);
					    }
					} else {
					    // Si 'attacker' no es un Player, maneja el caso según sea necesario.
					    // Por ejemplo, si es un summon, puedes manejarlo aquí.
					}
				}
			}
	 ...
	 */
	protected IntIntHolder calculateRewardItem(Player lastAttacker, DropData drop, int levelModifier, boolean isSweep)
	{
	 ...
	 * @return the ItemHolder.
	 */
	protected IntIntHolder calculateCategorizedRewardItem(Player lastAttacker, DropCategory categoryDrops, int levelModifier)
	{
	 ...
	 */
	protected int calculateLevelModifierForDrop(Player lastAttacker)
	{
		if (Config.DEEPBLUE_DROP_RULES)
		{
	...
	protected static IntIntHolder calculateCategorizedHerbItem(DropCategory categoryDrops, int levelModifier)
	{
		if (categoryDrops == null)
			return null;
			...
			if (cat.isSweep())
			{
				if (getSpoilerId() != 0)
				{
					for (DropData drop : cat.getAllDrops())
					{
						...
						if (item == null || player.ignoredDropContain(item.getId()))//if (item == null)
							continue;
