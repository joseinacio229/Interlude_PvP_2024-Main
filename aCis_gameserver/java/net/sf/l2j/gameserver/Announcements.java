...
public class Announcements
{
	...
	public static void ArenaAnnounce(String text)
	{
		...
		for (Player player : World.getInstance().getPlayers())
		{
			...
			{
				new CreatureSay(0, Say2.CRITICAL_ANNOUNCE, "", "[Tournament]: " + text);
			}
			if (Config.ARENA_MESSAGE_ENABLED)
			{
				if (player != null && player.isOnline() && (!player.isInsideZone(ZoneId.ARENA_EVENT)) && (!player.isDead()) && (!player.isInArenaEvent()) && (!player.isInStoreMode()) && (!player.isRooted()) && (player.getKarma() <= 0) && (!player.isInOlympiadMode())  && (!player.isFestivalParticipant()) && (Config.TOURNAMENT_EVENT_SUMMON))
				{
