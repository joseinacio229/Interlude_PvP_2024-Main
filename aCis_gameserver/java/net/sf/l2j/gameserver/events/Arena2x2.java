...
public class Arena2x2 implements Runnable
{
	...
	private List<Pair> selectOpponents() {
	    ...
	    while (tries > 0 && opponents.size() < 2) {
	        if (getRegisteredCount() < 2) {
	            return opponents;  // No hay suficientes registrados para hacer parejas
	        }

	        // Selecciona un par aleatorio
	        int index = Rnd.get(getRegisteredCount());
	        Pair candidate = registered.get(index);
	        
	        // Si el par es válido, lo añade a los oponentes y lo remueve de la lista registrada
	        if (candidate.check()) {
	            opponents.add(candidate);
	            registered.remove(index);
	        } else {
	            registered.remove(index);  // Si el par es inválido, simplemente lo remueve
	        }

	        tries--;
	    }

	    // Si se han encontrado dos oponentes válidos, los devuelve. De lo contrario, devuelve null
