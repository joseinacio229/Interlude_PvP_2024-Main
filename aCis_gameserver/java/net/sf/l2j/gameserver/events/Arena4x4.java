	...
	private List<Pair> selectOpponents() {
	    ...
	    do {
	        if (getRegisteredCount() < 2) {
	            return opponents;  // No hay suficientes registrados para hacer parejas
	        }
	        ...
	        if (pairOne == null) {
	            ...
	            if (pairOne.check()) {
	                ...
	                registered.remove(first); // Elimina el par de la lista de registrados
	            } else {
	                registered.remove(first); // Elimina el par si no es válido
	                return null;  // Termina y retorna null si el par es inválido
	            }
	        }
	        ...
	        if (pairTwo == null) {
	            ...
	            if (pairTwo.check()) {
	                ...
	                registered.remove(second); // Elimina el par de la lista de registrados
	            } else {
	                registered.remove(second); // Elimina el par si no es válido
	                return null;  // Termina y retorna null si el par es inválido
	            }
	        }

	        // Si ambos pares han sido seleccionados, sale del bucle
	        if (pairOne != null && pairTwo != null) {
	            break;
	        }

	        tries--;  // Reduce el contador de intentos
	    } while (tries > 0);

	    return opponents.size() == 2 ? opponents : null;  // Devuelve los oponentes si ambos fueron seleccionados, de lo contrario null
	}
	...
	private class EvtArenaTask implements Runnable
	{
		...
		private void rewardWinner()
		{
			if (pairOne.isAlive() && !pairTwo.isAlive())
			{
				...
				Broadcast.announceToOnlinePlayers("[4x4]: (" + leader1.getClan().getName() + " VS " + leader2.getClan().getName() + ") ~> " + leader1.getClan().getName() + " win!");
				pairOne.rewards();
				pairTwo.rewardsLost();
			}
