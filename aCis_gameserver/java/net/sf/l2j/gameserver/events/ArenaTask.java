...
public abstract class ArenaTask
{
	...
	public static void SpawnEvent()
	{
		...
		Broadcast.announceToOnlinePlayers("Reward: " + ItemTable.getInstance().getTemplate(Config.ARENA_REWARD_ID).getName());
		
		Broadcast.announceToOnlinePlayers("[Tournament]: Party Event PvP");
		Broadcast.announceToOnlinePlayers("[Tournament]: Teleport in the GK to (Tournament) Zone");
		Broadcast.announceToOnlinePlayers("[Tournament]: Duration: " + Config.TOURNAMENT_TIME + " minute(s)!");
	...
	public static void finishEvent()
	{
		...
		Broadcast.announceToOnlinePlayers("[Tournament]: Event Finished!");
		...
		for (Player player : World.getInstance().getPlayers())
			if (player != null && player.isOnline())
			{
				...
				new CreatureSay(0, Say2.CRITICAL_ANNOUNCE, "", "[Tournament] " + "Next Tournament: "  + ArenaEvent.getInstance().getNextTime() + " (GMT-3).");
	...
	public static void spawnNpc1()
	{
		...
		NpcTemplate template = NpcTable.getInstance().getTemplate(Config.ARENA_NPC);
		try
		{
    ...
    public static void spawnNpc2() {
        final NpcTemplate tmpl = NpcTable.getInstance().getTemplate(Config.ARENA_NPC);
        try {
            (ArenaTask._npcSpawn2 = new L2Spawn(tmpl)).setLoc(loc2x(), loc2y(), loc2z(), Config.NPC_Heading);
            ArenaTask._npcSpawn2.setRespawnDelay(1);
            SpawnTable.getInstance().addNewSpawn(ArenaTask._npcSpawn2, false);
			...
			switch (seconds)
			{
				case 3600:
					if (_started)
					{
						...
						Broadcast.announceToOnlinePlayers("[Tournament]: Party Event PvP");
						Broadcast.announceToOnlinePlayers("[Tournament]: Teleport in the GK to (Tournament) Zone");
						Broadcast.announceToOnlinePlayers("[Tournament]: Reward: " + ItemTable.getInstance().getTemplate(Config.ARENA_REWARD_ID).getName());
						Broadcast.announceToOnlinePlayers("[Tournament]: " + seconds / 60 / 60 + " hour(s) till event finish!");
					}
					break;
				case 60:
				case 120:
				...
				case 1800:
					if (_started)
						Broadcast.announceToOnlinePlayers("[Tournament]: " + seconds / 60 + " minute(s) till event finish!");
					break;
				case 1:
				case 2:
				case 3:
				...
				case 30:
					if (_started)
						Broadcast.announceToOnlinePlayers("[Tournament]: " + seconds + " second(s) till event finish!");
					break;
			}
