...
public class Arena9x9 implements Runnable
{
	...
	private List<Pair> selectOpponents() {
	    ...
	    do {
	        if (getRegisteredCount() < 2) {
	            return opponents;  // No hay suficientes registrados para seleccionar oponentes
	        }

	        // Selecciona el primer par si no está ya seleccionado
	        if (pairOne == null) {
	            int first = Rnd.get(getRegisteredCount());
	            pairOne = registered.get(first);

	            if (pairOne.check()) {
	                opponents.add(pairOne);  // Añade el primer par a la lista de oponentes
	                registered.remove(first); // Remueve el par de la lista de registrados
	            } else {
	                registered.remove(first);  // Remueve el par si es inválido
	                return null;  // Devuelve null si el primer par no es válido
	            }
	        }

	        // Selecciona el segundo par si no está ya seleccionado
	        if (pairTwo == null) {
	            int second = Rnd.get(getRegisteredCount());
	            pairTwo = registered.get(second);

	            if (pairTwo.check()) {
	                opponents.add(pairTwo);  // Añade el segundo par a la lista de oponentes
	                registered.remove(second); // Remueve el par de la lista de registrados
	            } else {
	                registered.remove(second);  // Remueve el par si es inválido
	                return null;  // Devuelve null si el segundo par no es válido
	            }
	        }

	        // Si ambos pares han sido seleccionados, se sale del bucle
	        if (pairOne != null && pairTwo != null) {
	            break;
	        }

	        tries--;  // Decrementa el contador de intentos
	    } while (tries > 0);

	    // Devuelve la lista de oponentes si ambos pares son válidos, de lo contrario, null
	    return opponents.size() == 2 ? opponents : null;
	}
	...
	private class Pair
	{
		...
		public void teleportTo(int x, int y, int z)
		{
			if (leader != null && leader.isOnline())
			{
				leader.getAppearance().getInvisible();
			...
			if (assist != null && assist.isOnline())
			{
				assist.getAppearance().getInvisible();
			...
			if (assist2 != null && assist2.isOnline())
			{
				assist2.getAppearance().getInvisible();
			...
			if (assist3 != null && assist3.isOnline())
			{
				assist3.getAppearance().getInvisible();
			...
			if (assist4 != null && assist4.isOnline())
			{
				assist4.getAppearance().getInvisible();
			...
			if (assist5 != null && assist5.isOnline())
			{
				assist5.getAppearance().getInvisible();
			...
			if (assist6 != null && assist6.isOnline())
			{
				assist6.getAppearance().getInvisible();
			...
			if (assist7 != null && assist7.isOnline())
			{
				assist7.getAppearance().getInvisible();
			...
			if (assist8 != null && assist8.isOnline())
			{
				assist8.getAppearance().getInvisible();
	...
	private class EvtArenaTask implements Runnable
	{
		...
		private void rewardWinner()
		{
			if (pairOne.isAlive() && !pairTwo.isAlive())
			{
				...
				Broadcast.announceToOnlinePlayers("[9x9]: (" + leader1.getClan().getName() + " VS " + leader2.getClan().getName() + ") ~> " + leader1.getClan().getName() + " win!");
