        ...
        for (WorldObject obj : targets)
        {
            ...
            for (L2Effect effect : target.getAllEffects())
            {
                ...
                if (calcCancelSuccess(effect.getPeriod(), diffLevel, skillPower, skillVuln, minRate, maxRate))
                {
                 ...
                 // Custom cancel
                    if (!cancelledBuffs.contains(effect.getSkill()) && !target.isDead() && Config.CANCEL_RETURN)
                        cancelledBuffs.add(effect.getSkill());

                    // Programar la tarea de retorno de cancelación al objetivo
                    if (cancelledBuffs.size() > 0 && Config.CANCEL_RETURN) {
                        ThreadPool.schedule(new CancelTaskManager(target, cancelledBuffs), Config.CANCEL_SECONDS * 1000);
                    }
